...
def interpolate(mesh_name, prim_arr, sim_time, imgsize):
    ...
    i = 0
    p_ipol = np.zeros((imgsize[0],imgsize[1]))
    Ux_ipol = np.zeros((imgsize[0],imgsize[1]))
    Uy_ipol = np.zeros((imgsize[0],imgsize[1]))
    for xi in range(0,imgsize[1]):
        for yi in range(0,imgsize[0]):
            if prim_arr[xi,yi] == 0:
                ...
                p_ipol[yi,xi] = 0 #np.nan
                Ux_ipol[yi,xi] = 0 #np.nan
                Uy_ipol[yi,xi] = 0 #np.nan
                     
        
    # crop the generated arrays to region of interest
    Ux_ipol = Ux_ipol[0:imgsize[0], 0:imgsize[1]]
    Uy_ipol = Uy_ipol[0:imgsize[0], 0:imgsize[1]]
    p_ipol = p_ipol[0:imgsize[0], 0:imgsize[1]]

    # stacking the generated arrays
    bundle = np.stack((Ux_ipol,Uy_ipol,p_ipol))
