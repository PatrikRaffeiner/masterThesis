...
y_NACA_smooth = pickle.load(open("y_dataSmooth.pkl","rb"))
x_NACA_smooth = pickle.load(open("x_dataSmooth.pkl","rb"))
y_NACA_smooth = y_NACA_smooth[0]
x_NACA_smooth = x_NACA_smooth[0]
y_NACA_pixelated = pickle.load(open("y_dataPixelated.pkl","rb"))
x_NACA_pixelated = pickle.load(open("x_dataPixelated.pkl","rb"))
...
plt.subplot(2,2,1)
plt.imshow(y_NACA_smooth[0,:,:].T, cmap="turbo", vmin=vmin, vmax=vmax)
plt.title('Smooth geometry', fontsize=18)
plt.gca().invert_yaxis()
plt.colorbar(orientation='horizontal')
plt.subplot(2,2,2)
plt.imshow(y_NACA_pixelated[0,:,:].T, cmap="turbo", vmin=vmin, vmax=vmax)
plt.title('Pixelated geometry', fontsize=18)
plt.gca().invert_yaxis()
plt.colorbar(orientation='horizontal')
plt.subplot(2,2,3)
plt.imshow(error[0,:,:].T, cmap="turbo")
